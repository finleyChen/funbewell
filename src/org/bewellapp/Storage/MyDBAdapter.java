...
public class MyDBAdapter {
	...
	private static final long MAX_INSERT_PER_TRANSACTION = 50;
	...
	private static final String CREATE_TABLE_INFERENCE = "CREATE TABLE IF NOT EXISTS "
			+ "inference(inference_time DATETIME ASC, inference_is_start BOOLEAN, inference_type INTEGER, sync_id INTEGER, inference_name TEXT);";
	...
	public long getNewEventID() {
		if (lastEventId < 0) {
			Cursor c = db.query(true, TABLE_NAME_EVENTS, new String[] { "event_id" }, null, null, null, null, "event_id desc", "1");
			if (c.moveToNext()) {
				lastEventId = c.getLong(0);
			} else {
				lastEventId = 0;
			}
			c.close();
	...
	public void registerSource(String source_name, int source_sampling_rate) {
		...
		// Cursor cur = db.query(TABLE_NAME_SOURCES, new String[] { "source_id"
		// }, "source_name=? AND source_sampling_rate=?",
		// new String[] {source_name, Integer.toString(source_sampling_rate)},
		// null, null, null);
	...
	public void insertEntryEvents(long event_time, int event_source_id, int event_source_message_type_id,
			byte[] event_data, int sync_id) {
		ContentValues value = new ContentValues();
	...
	public void insertEntryInference(long inference_time, boolean inference_is_start, int inference_type,
			String inference_name, int sync_id) {
		ContentValues value = new ContentValues();
	...
	public void insertEntryLocation(long location_time, boolean location_is_start, double location_lat,
			double location_lon) {
	...
	private long insertEntry(String table_name, ContentValues value) {
		...
		try {
			...
			if (insert_count == MAX_INSERT_PER_TRANSACTION) {
				Log.d("BeWell DB Adapter", "Transaction sucessful");
	...
	public void insertMltObj(Object obj) {
		...
		// 0 = audio, 1=accelerometer, 2=location, 3 = audio features, 4 = accel
		// features, 5 = audio inference, 6 = accel inference, 7 = label_start ,
		// 8 = label_end

		if (mltobj.datatype < 5 || mltobj.datatype > 8)// means no inference
														// result or label
														// information
			...
			insertEntryInference(mltobj.timestamp, mltobj.isStart, 0, mltobj.inferred_results_or_label_info,
					mltobj.sync_id); // 0 means audio inference
		else if (mltobj.datatype == 6) // go to inferred results
			insertEntryInference(mltobj.timestamp, mltobj.isStart, 1, mltobj.inferred_results_or_label_info,
					mltobj.sync_id); // 1 means accel inference
		else if (mltobj.datatype == 7) // labels
			insertEntryLabel(mltobj.timestamp, true, 0, mltobj.inferred_results_or_label_info);
		else if (mltobj.datatype == 8) // labels
			insertEntryLabel(mltobj.timestamp, false, 0, mltobj.inferred_results_or_label_info);
	}

	/*
	 * public boolean removeEntry(long _rowIndex) { return
	 * db.delete(DATABASE_TABLE, KEY_ID + "=" + _rowIndex, null) > 0; }
	 */
	public boolean removeAllEntries() {
		return db.delete(TABLE_NAME_SOURCES, null, null) > 0
				&& db.delete(TABLE_NAME_EVENTS, null, null) > 0 && db.delete(TABLE_NAME_INFERENCES, null, null) > 0
				&& db.delete(TABLE_NAME_LABEL, null, null) > 0 && db.delete(TABLE_NAME_LOCATION, null, null) > 0
				&& db.delete(TABLE_NAME_EXPERIMENTS, null, null) > 0;

	}

	/*
	 * public Cursor getAllEntries() { return db.query(DATABASE_TABLE, new
	 * String[] { KEY_ID, KEY_NAME }, null, null, null, null, null); }
	 * 
	 * public MyObject getEntry(long _rowIndex) { // TODO: Return a cursor to a
	 * row from the database and // use the values to populate an instance of
	 * MyObject
	 * 
	 * String selection = KEY_ID + "=" + _rowIndex; Cursor cur =
	 * db.query(DATABASE_TABLE, new String[] { KEY_ID, KEY_NAME }, selection,
	 * null, null, null, null);
	 * 
	 * ContentValues values = new ContentValues(); cur.moveToFirst(); int id =
	 * cur.getInt(0); String name = cur.getString(1); values.put(KEY_ID,id);
	 * values.put(KEY_NAME,name);
	 * 
	 * MyObject objectInstance = new MyObject(values); return objectInstance; }
	 * 
	 * public boolean updateEntry(long _rowIndex, MyObject _myObject) { // TODO:
	 * Create a new ContentValues based on the new object // and use it to
	 * update a row in the database. return true; }
	 */
 ...
 * android.util.Log;
 * 
 * public class MyDBAdapter { private String DATABASE_NAME;// = "myDatabase.db";
 * private static final String DATABASE_TABLE = "mainTable"; private static
 * final int DATABASE_VERSION = 1;
 * 
 * // The index (key) column name for use in where clauses. public static final
 * String KEY_ID = "_id";
 * 
 * // The name and column index of each column in your database. public static
 * final String KEY_NAME = "data"; public static final int NAME_COLUMN = 1; //
 * TODO: Create public field for each column in your table.
 * 
 * 
 * private long insert_count; private static final long
 * MAX_INSERT_PER_TRANSACTION = 500;//0; private ML_toolkit_object mlobj;
 * 
 * 
 * // SQL Statement to create a new database. private static final String
 * DATABASE_CREATE = "create table " + DATABASE_TABLE + " (" + KEY_ID +
 ...
 * public boolean database_online;
 * 
 * public MyDBAdapter(Context _context, String database_name) { context =
 * _context;
 * 
 * this.DATABASE_NAME = database_name; dbHelper = new myDbHelper(context,
 * DATABASE_NAME, null, DATABASE_VERSION); }
 * 
 * public MyDBAdapter open() throws SQLException { db =
 * dbHelper.getWritableDatabase(); database_online = true; return this; }
 * 
 * public void close() { database_online = false;
 * //db.setTransactionSuccessful(); insert_count=0; //db.endTransaction();
 * db.close(); }
 * 
 * public long getDbSize() { File file = new File(this.getPathOfDatabase());
 * return file.length(); }
 * 
 * public int insertEntry(Object obj) { // TODO: Create a new ContentValues to
 * represent my row // and insert it into the database.
 * 
 * 
 * 
 * 
 * ContentValues values = _myObject.getValue(); int index = 0;
 * 
 * /* Log.d("XY", "Insersion count "+insert_count); index =
 * (int)db.insert(DATABASE_TABLE, null, values); insert_count++;
 */

/*
 * try{ if (insert_count == 0) db.beginTransaction();
 * 
 * index = (int)db.insert(DATABASE_TABLE, null, values); insert_count++;
 * 
 * //Log.d("XY", "Insersion count "+insert_count); if(insert_count ==
 * MAX_INSERT_PER_TRANSACTION ) { //Log.d("XY", "trasaction sucessful");
 * 
 * db.setTransactionSuccessful(); } }catch (SQLException e) { } finally {
 * if(insert_count == MAX_INSERT_PER_TRANSACTION ){ insert_count=0;
 * db.endTransaction(); } }
 * 
 * return index; }
 * 
 * public boolean removeEntry(long _rowIndex) { return db.delete(DATABASE_TABLE,
 * KEY_ID + "=" + _rowIndex, null) > 0; }
 * 
 * public String getPathOfDatabase() { return db.getPath(); }
 * 
 * public boolean removeAllEntries() { return db.delete(DATABASE_TABLE, null,
 * null) > 0; }
 * 
 * public Cursor getAllEntries() { return db.query(DATABASE_TABLE, new String[]
 * { KEY_ID, KEY_NAME }, null, null, null, null, null); }
 * 
 * public MyObject getEntry(long _rowIndex) { // TODO: Return a cursor to a row
 * from the database and // use the values to populate an instance of MyObject
 * 
 * String selection = KEY_ID + "=" + _rowIndex; Cursor cur =
 * db.query(DATABASE_TABLE, new String[] { KEY_ID, KEY_NAME }, selection, null,
 * null, null, null);
 * 
 * ContentValues values = new ContentValues(); cur.moveToFirst(); int id =
 * cur.getInt(0); String name = cur.getString(1); values.put(KEY_ID,id);
 ...
 * 
 * public boolean updateEntry(long _rowIndex, MyObject _myObject) { // TODO:
 * Create a new ContentValues based on the new object // and use it to update a
 * row in the database. return true; }
 * 
 * private static class myDbHelper extends SQLiteOpenHelper {
 * 
 * public myDbHelper(Context context, String name, CursorFactory factory, int
 * version) { super(context, name, factory, version); }
 * 
 ...
 * version.
 * 
 * @Override public void onUpgrade(SQLiteDatabase _db, int _oldVersion, int
 * _newVersion) { // Log the version upgrade. Log.w("TaskDBAdapter",
 * "Upgrading from version " + _oldVersion + " to " + _newVersion +
 * ", which will destroy all old data");
 * 
 ...
 * 
 * // The simplest case is to drop the old table and create a new one.
 * _db.execSQL("DROP TABLE IF EXISTS " + DATABASE_TABLE); // Create a new one.
 * onCreate(_db); } }
 * 
 * 
 * }
