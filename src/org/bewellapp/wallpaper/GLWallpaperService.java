...
//Code added by Jennifer Lure to incorporate touch events
...
import org.bewellapp.wallpaper.BaseConfigChooser.ComponentSizeChooser;
import org.bewellapp.wallpaper.BaseConfigChooser.SimpleEGLConfigChooser;
...
public class GLWallpaperService extends WallpaperService {
	...
	public class GLEngine extends Engine {
		...
		// Added by Jennifer Lure 9/3/2011
		@Override
		public Bundle onCommand(String action, int x, int y, int z, Bundle extras, boolean resultRequested) {
			if (action.equals(WallpaperManager.COMMAND_TAP) && mGLThread.getRenderer().touchTrigger(x, y)) {
				BeWellFishRenderer renderer = (BeWellFishRenderer) mGLThread.getmRenderer();
				Intent intent = new Intent(GLWallpaperService.this, WellnessSummaryActivity.class);
				intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				intent.putExtra(ScoreComputationService.BEWELL_SCORE_PHYSICAL, renderer.mPhysAct);
				intent.putExtra(ScoreComputationService.BEWELL_SCORE_SOCIAL, renderer.mSocialAct);
				intent.putExtra(ScoreComputationService.BEWELL_SCORE_SLEEP, renderer.mSleep);
				intent.putExtra(ScoreComputationService.BEWELL_SCORE_PHYSICAL_OLD, renderer.mPhysActOld);
				intent.putExtra(ScoreComputationService.BEWELL_SCORE_SOCIAL_OLD, renderer.mSocialActOld);
				intent.putExtra(ScoreComputationService.BEWELL_SCORE_SLEEP_OLD, renderer.mSleepOld);
		...
		public void onOffsetsChanged(float xOffset, float yOffset, float xOffsetStep, float yOffsetStep,
			...
			mGLThread.getmRenderer().onOffsetsChanged(xOffset, yOffset, xOffsetStep, yOffsetStep, xPixelOffset,
					yPixelOffset);
		}
	...
	public interface Renderer {
		...
		// added by Jennifer Lure on 9/7/11
...
class GLThread extends Thread {
		...
		try {
			...
			while (!isDone()) {
				...
				if (tellRendererSurfaceCreated) {
					getmRenderer().onSurfaceCreated(gl, mEglHelper.mEglConfig);
					tellRendererSurfaceCreated = false;
				}
				if (tellRendererSurfaceChanged) {
					getmRenderer().onSurfaceChanged(gl, w, h);
					tellRendererSurfaceChanged = false;
				}
				if ((w > 0) && (h > 0)) {
					/* draw a frame here */
					getmRenderer().onDrawFrame(gl);
	...
	public void setRenderer(GLWallpaperService.Renderer mRenderer) {
		this.mRenderer = mRenderer;
	}

	public GLWallpaperService.Renderer getRenderer() {
		return mRenderer;
	}

	public void setmRenderer(GLWallpaperService.Renderer mRenderer) {
		this.mRenderer = mRenderer;
	}

	public GLWallpaperService.Renderer getmRenderer() {
		return mRenderer;
	}
